                ...
                if (pingData.size() > 0)
                {
                    ...
                    else if (pingData[0] == '2')
                    {
                      //  Serial.println("No\nroute");
                        ...
                        if (pingData.length() > speedOffset)
                            Serial.println(pingData.c_str()[speedOffset]);
                    ...
                    else if (pingData[0] == 'i')
                    {
                        ...
                        if (pingData.length() > distanceOffset)
                            stringDistance = (pingData.c_str() + distanceOffset);
                           
                        if (pingData.length() > instructionOffset)
                            direction = (pingData.c_str()[instructionOffset]);

                        if (pingData.length() > speedOffset)
                            speed = (pingData.c_str()[speedOffset]);
...
//----------------------------------IF SPEED CHANGES - SEND IT TO SERIAL

if (sygicSpeed != lastsygicSpeed){
  lastsygicSpeed = sygicSpeed;
  if (sygicSpeed > 0){
      Serial.println(sygicSpeed);
  }
}

sygicSpeed = speed;

int distance = stringDistance.toInt(); 
//----------------------------------SPEED REDUCTION FOR EXIT RIGHT
if (direction == 43)
   {
